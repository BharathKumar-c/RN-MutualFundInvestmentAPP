...
const EmploymentStatus = ({
  ...
  const renderItem = ({ item }) => {
    const backgroundColor = item.name === selectedId ? '#EDEDE7' : '#EDEDE7';
    const color = item.name === selectedId ? '#333333' : '#333333';
  ...
  return (
    <Box style={styles.container}>
      ...
      <Box style={styles.continueBtn}>
        <AbstractButton
          onPress={() => {
            setEmploymentStatusName(selectedId);
            handleClosePress();
          }}>
          Continue
        </AbstractButton>
